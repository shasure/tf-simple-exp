...
tf.flags.DEFINE_string('train_dir', '/train_dir/wgan-mnist', 'events and ckpt dir')
...
class WGANmnist(object):
    ...
    def loss(self):
        ...
        d_loss = tf.reduce_mean(x_logit - z_logit)
    ...
    def train(self):
        ...
        grads_d = tf.gradients(-d_loss, param_d)  # use -d_loss
        ...
        var_clip_ops = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in param_d]
        ...
        with tf.Session() as sess:
            ...
            for local_step in range(FLAGS.num_steps):
                ...
                for k in range(self.critic):
                    ...
                    _, d_loss_str, global_step_str, _ = sess.run(
                        [train_op_d, d_loss, global_step, var_clip_ops], feed_dict=feed_dict)
                # training G
                batch_x, _ = self.mnist.train.next_batch(FLAGS.batch_size)
                feed_dict = {self.x: batch_x}
